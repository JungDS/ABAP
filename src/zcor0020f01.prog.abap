...
FORM SCR_USER_COMMAND .
  ...
  CASE SSCRFIELDS-UCOMM.
    ...
    WHEN 'FC02'.
      PERFORM EXCELFORM_DOWNLOAD_V1 USING 'ZCOR0020'.
...
FORM DATA_CONVERT .
  ...
  LOOP AT GT_EXCEL.
    ...
    MOVE: GT_EXCEL-KOSTL TO GS_OUTTAB-KOSTL,
...
FORM CHECK_DATA USING VALUE(P_PLUS).
  ...
  "-- 필수 값 체크
  _INITIAL_CHK : GS_OUTTAB-KOSTL  TEXT-C02,
                 GS_OUTTAB-KSTAR  TEXT-C04,
*                 GS_OUTTAB-BUKRS  TEXT-001,
*                 GS_OUTTAB-PRCTR  TEXT-C09,
  ...
  "-- 1) 코스트 센터
  SELECT SINGLE A~BUKRS, A~PRCTR, B~KTEXT
    INTO @DATA(LS_CSKS)
    FROM CSKS AS A
    LEFT JOIN CSKT AS B
      ON A~KOKRS = B~KOKRS
   ...
   WHERE A~KOKRS = @PA_KOKRS
     AND A~KOSTL = @GS_OUTTAB-KOSTL
     AND A~DATBI >= @SY-DATUM
     AND A~DATAB <= @SY-DATUM.

  IF SY-SUBRC <> 0 .
    GS_OUTTAB-STATUS = 'E'.
    MESSAGE S023 INTO GS_OUTTAB-MESSAGE WITH GS_OUTTAB-KOSTL.
    EXIT.
  ELSE.

    MOVE LS_CSKS-KTEXT TO GS_OUTTAB-KTEXT.
...
*-- CO 계획: 1차 원가 BAPI에 대한 오브젝트
    ...
    LT_COOBJECT-COSTCENTER   = GS_OUTTAB-KOSTL.
    APPEND LT_COOBJECT.
...
FORM BUILT_HEADER CHANGING CR_CONTENT TYPE REF TO CL_SALV_FORM_ELEMENT.
  ...
  CONCATENATE TEXT-007 ':' PA_SPERL '~' PA_EPERL
      INTO L_TEXT SEPARATED BY SPACE.
