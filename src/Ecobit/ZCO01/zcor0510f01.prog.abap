...
*WBS
  SELECT posid ,  post1, pbukr AS bukrs
    FROM prps
...
*HR  수신  부서 : WBS   /  부서 : CCTR

  SELECT a~kostl  AS kostl,
         a~posid AS posid,
         a~dept  AS dept,
         b~dept_txt AS dept_txt
   FROM zfit0300 AS a LEFT JOIN zfit0310 AS b
    ON a~dept = b~dept
   WHERE a~bukrs IN @so_bukrs
   INTO TABLE @DATA(lt_0310).
...
**CCTR..
  ...
  LOOP AT  lt_csks ASSIGNING FIELD-SYMBOL(<fs>).

    READ TABLE gt_display ASSIGNING FIELD-SYMBOL(<fs2>)
                          WITH KEY  kostl = <fs>-kostl.
    IF sy-subrc EQ 0.
      <fs2>-ktext = <fs>-ktext. "CCTR 명
      <fs2>-bukrs = <fs>-bukrs.

      IF <fs2>-dept IS INITIAL. " 기존 부서 매칭이 없으면 HR에서 들어온 매핑을 넣어준다

        READ TABLE lt_0310 ASSIGNING FIELD-SYMBOL(<fs3>) WITH KEY kostl = <fs>-kostl.
        IF sy-subrc EQ 0.
          <fs2>-dept     = <fs3>-dept .
          <fs2>-dept_txt = <fs3>-dept_txt .
        ENDIF.

      ELSE. "부서명 넣기

        <fs2>-icon = icon_led_green.

        SELECT SINGLE dept_txt
          FROM zfit0310
         WHERE spras = 'KO'
           AND dept = @<fs2>-dept
          INTO  @<fs2>-dept_txt.
    ...
    ELSE.  " 없으면 그냥 APPEND..
      ...
      gs_display-kostl = <fs>-kostl.
      gs_display-ktext = <fs>-ktext.

      gs_display-bukrs = <fs>-bukrs.
      gs_display-icon = icon_led_yellow.


      READ TABLE lt_0310 ASSIGNING FIELD-SYMBOL(<fs4>) WITH KEY kostl = <fs>-kostl.
      IF sy-subrc EQ 0.
        gs_display-dept     = <fs4>-dept.  " 부서명
        gs_display-dept_txt = <fs4>-dept_txt .
      ENDIF.
...
*WBS..
  LOOP AT  lt_prps ASSIGNING FIELD-SYMBOL(<ps>).

    READ TABLE gt_display ASSIGNING FIELD-SYMBOL(<ps2>)
                          WITH KEY  posid = <ps>-posid.
    IF sy-subrc EQ 0.
      <ps2>-post1 = <ps>-post1.
      <ps2>-bukrs = <ps>-bukrs.

      IF <ps2>-dept IS INITIAL.   "기존 부서 매칭이 없으면 HR에서 들어온 매핑을 넣어준다

        READ TABLE lt_0310 ASSIGNING FIELD-SYMBOL(<ps3>) WITH KEY posid = <ps>-posid.
        IF sy-subrc EQ 0.
          <ps2>-dept     = <ps3>-dept.  " 부서명
          <ps2>-dept_txt = <ps3>-dept_txt .
        ENDIF.

      ELSE. "부서명 넣기
        <ps2>-icon = icon_led_green.
        SELECT SINGLE dept_txt
          FROM zfit0310
         WHERE spras = 'KO'
           AND dept = @<ps2>-dept
    ...
    ELSE.  " 없으면 그냥 APPEND..
      ...
      CLEAR gs_display.
      gs_display-kokrs = pa_kokrs.
      gs_display-posid = <ps>-posid.
      gs_display-post1 = <ps>-post1.
      gs_display-bukrs = <ps>-bukrs.
      gs_display-icon  = icon_led_yellow.

      READ TABLE lt_0310 ASSIGNING FIELD-SYMBOL(<ps4>) WITH KEY posid = <ps>-posid.
      IF sy-subrc EQ 0.
        gs_display-dept     = <ps4>-dept.  " 부서명
        gs_display-dept_txt = <ps4>-dept_txt .
      ENDIF.
  ...
  IF so_bukrs[]  IS NOT INITIAL.
    DELETE gt_display WHERE bukrs NOT IN so_bukrs.
  ENDIF.
...
*--- Begin or Example
  ...
  DATA lv_dept TYPE zdept.
  DATA lv_dept_txt TYPE zdept_txt.
      ...
      LOOP AT pr_data_changed->mt_mod_cells INTO ls_mod_cells.
        ...
        CASE ls_mod_cells-fieldname.
          ...
          WHEN 'DEPT'. " 부서
            ...
            _get_cell_value 'BUKRS' ls_mod_cells-row_id
                                             lv_bukrs.

            IF ls_mod_cells-value IS NOT INITIAL.

              lv_dept = ls_mod_cells-value.

              SELECT SINGLE dept_txt INTO @lv_dept_txt
                FROM zfit0310
               WHERE dept = @lv_dept
                 AND bukrs = @lv_bukrs.
              ...
              IF sy-subrc <> 0.
                _add_protocol ls_mod_cells-fieldname ls_mod_cells-row_id
                              'E' 023 TEXT-c99 space space space.
                gv_exit = abap_true . EXIT.
              ENDIF.
            ENDIF.

            _modify_cell 'DEPT_TXT' ls_mod_cells-row_id
                                 lv_dept_txt.
  ...
  CASE pv_fieldname.
    WHEN 'DEPT'.
      ...
      IF sy-subrc EQ 0.
        ...
        SELECT dept, dept_txt
          FROM zfit0310
          WHERE bukrs =  @gs_display-bukrs
            AND spras = 'KO'
         INTO TABLE @gt_dept.
...
FORM get_fieldcatlog_data .
  ...
  CALL FUNCTION 'K_KKB_FIELDCAT_MERGE'
    EXPORTING
      ...
      i_strucname            = 'ZCOS0510' "ABAP DIC. 정의된 STRUCTURE
...
FORM dele_select_data .
  ...
  DATA : lt_2000 TYPE TABLE OF zcot2000,
         ls_2000 LIKE LINE OF lt_2000.

  DATA : lt_2000log TYPE TABLE OF zcot2000log,
         ls_2000log LIKE LINE OF lt_2000log.
...
*     PARAMETER             =
  ...
  LOOP AT gt_rows INTO gs_rows.
    ...
    INTO CORRESPONDING FIELDS OF @ls_2000.
    IF sy-subrc EQ 0.

      DELETE   FROM zcot2000
               WHERE kokrs  = @pa_kokrs
                AND kostl  = @<fs_disp>-kostl
                AND posid  = @<fs_disp>-posid.
      ...
      IF sy-subrc EQ 0.
        ...
        CLEAR ls_2000log.
        MOVE-CORRESPONDING <fs_disp> TO ls_2000log.

        ls_2000log-tstampl = sy-datum && sy-uzeit && lv_timestamp.

        ls_2000log-flag = 'D'.
        ls_2000log-erdat = sy-datum.  "삭제 실행일
        ls_2000log-erzet = sy-uzeit.
        ls_2000log-ernam = sy-uname.

        INSERT zcot2000log FROM ls_2000log.
  ...
  LOOP AT gt_display ASSIGNING <fs_disp> WHERE mark = 'X'.
    ...
    CASE  sy-subrc.
      WHEN 0.   " 수정 UPDATE..
        ...
        CLEAR ls_2000.
        MOVE-CORRESPONDING <fs_disp> TO ls_2000.
        ls_2000-kokrs = pa_kokrs.

        ls_2000-aedat = sy-datum.
        ls_2000-aezet = sy-uzeit.
        ls_2000-aenam = sy-uname.

        MODIFY  zcot2000 FROM ls_2000.
        ...
        IF sy-subrc EQ 0.
          ...
          CLEAR ls_2000log.
          MOVE-CORRESPONDING ls_2000 TO ls_2000log.

          ls_2000log-tstampl = sy-datum && sy-uzeit && lv_timestamp.

          ls_2000log-flag = 'U'.
          ls_2000log-kokrs = pa_kokrs.
          ls_2000log-aedat = sy-datum.
          ls_2000log-aezet = sy-uzeit.
          ls_2000log-aenam = sy-uname.

          MODIFY zcot2000log FROM ls_2000log.
      ...
      WHEN OTHERS.  " 신규 저장  INSERT .
        ...
        ls_2000-erdat = sy-datum.
        ls_2000-erzet = sy-uzeit.
        ls_2000-ernam = sy-uname.
