...
**  선택 회사별  objnr..
  ...
  CLEAR :  gv_super, gv_bukrs_auth.  CLEAR gv_super_bukrs.
...
*  PERFORM SELECTED_MAIN_DATA.
  ..
*<<<<<<<<<<<<<<<<<     MODI BSGSm_FCM  20210325


  IF gv_error IS INITIAL AND ( r_objnr[] IS NOT INITIAL OR gv_super EQ abap_true ).
  ...
  ELSE.  " 슈퍼가 아니면..
    ...
    IF sy-subrc EQ 0.
      ...
      IF lr_pspnr IS INITIAL AND
        ...
        gv_error =  TEXT-e88. "'선택한 회사코드 조회권한이 없습니다.'.
        ...
        LOOP AT gt_values.
          ...
          SELECT a~objnr, a~pspnr, a~posid,  a~post1, a~zzcyp
                    ...
                    WHERE a~pkokr = @pa_kokrs
                      AND a~pbukr IN @lr_bukrs " <<<<<<<<<<
*                      AND a~posid IN @lr_pspnr  " <<<   권한 wbs...
*                      AND a~posid IN @so_posid.   "<<< input wbs...
                     AND  a~posid BETWEEN  @gt_values-vfrom  AND @gt_values-vto.
        ...
        LOOP AT gt_values.
          ...
          SELECT a~objnr, a~pspnr, a~posid,  a~post1, a~zzcyp
                    ...
                    INNER JOIN proj AS b
                       ON a~psphi = b~pspnr
                    WHERE a~pkokr = @pa_kokrs
                      AND a~prctr IN @lr_prctr " <<<  권한 PCTR
*                      AND a~posid IN @lr_pspnr  " <<<   권한 wbs...
*                      AND a~posid IN @so_posid.   "<<< input wbs...
                     AND  a~posid BETWEEN  @gt_values-vfrom  AND @gt_values-vto.
...
**wbs..
    ...
    ENDIF.  " lr_prctr[]  end if 문...

*권한 있는  WBS,
*우선 순위  3 .  특정 WBS  권한 있을 때..
...
* END BY BSGSM_FCM.
  ...
  SELECT b~posid,
    ...
    FROM cosp AS a INNER JOIN prps AS b
                           ON a~objnr = b~objnr
*                          AND B~OBJNR LIKE 'PR&'
                    LEFT JOIN csku AS c
                           ON c~spras = @sy-langu
   ...
   WHERE a~lednr    = '00'
     ...
     AND a~objnr   IN @r_objnr
     AND a~objnr LIKE 'PR%'
   GROUP BY b~posid, b~post1, a~kstar, c~ktext, a~twaer.


  LOOP AT lt_sum INTO DATA(ls_sum).


    MOVE: ls_sum-posid TO gs_display-posid,
          ls_sum-post1 TO gs_display-post1,
          ls_sum-kstar TO gs_display-kstar,
          ls_sum-ktext TO gs_display-ktext,
    ...
    IF ls_sum-kstar CP '04*'   OR ls_sum-kstar CP '0701*' OR
      ...
      MOVE TEXT-006 TO gs_display-kstxt.
    ENDIF.
...
*<<<<<<<<<<<<< ADD BSGSM_FCM 20210325
    ...
    READ TABLE  lt_prps ASSIGNING FIELD-SYMBOL(<fs>) WITH KEY posid = gs_display-posid .
    IF sy-subrc EQ 0.
...
*    APPEND gs_display TO gt_display.
  ...
  SORT gt_display BY posid kstar.
...
FORM get_fieldcatlog_data .
  ...
  CALL FUNCTION 'K_KKB_FIELDCAT_MERGE'
    EXPORTING
      ...
      i_strucname            = 'ZCOS0191' "ABAP DIC. 정의된 STRUCTURE
...
FORM excel_download .
  ...
  LOOP AT t_fieldcat INTO gs_fieldcat.
    ...
    CASE gs_fieldcat-fieldname.
      ...
      WHEN 'POST1' OR 'KSTXT' OR 'TOTAL' OR 'MESSAGE'.
        gs_fieldcat-no_out = abap_true.
