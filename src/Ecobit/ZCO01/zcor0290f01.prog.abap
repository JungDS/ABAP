...
FORM DATA_GET .
  ...
  DATA: LV_F3 TYPE HSLXX9_CS,
        LV_F2 TYPE HSLXX9_CS,
        LV_F1 TYPE HSLXX9_CS.
...
**END BY SGSM_FCM
  ...
  SELECT * FROM ZCOT0030
    INTO TABLE @DATA(LT_ZCOT0030)
    WHERE GJAHR = @PA_GJAHR
      AND KOKRS = @PA_KOKRS
      AND BEXCL = @ABAP_TRUE.

  SELECT A~KOSTL, B~KTEXT,
        A~BUKRS AS BUKRS "ADD BSGSM_FCM
    INTO TABLE @GT_KOSTL
    FROM CSKS AS A
    LEFT JOIN CSKT AS B
      ON A~KOKRS = B~KOKRS
...
*-- 계획
 ...
 WHERE A~RLDNR = '00'
  ...
  GROUP BY A~RYEAR, A~ROBJNR, A~RKSTAR, B~KTEXT, A~PROCESS_9, A~RTCUR.
...
*-- F3
  SELECT OBJNR, KSTAR,
   ...
   WHERE LEDNR = '00'
     ...
     AND VRGNG = 'RKU1'
   GROUP BY GJAHR, OBJNR, KSTAR.

*-- F1/F2
  SELECT RACCT, OBJNR, BLART, FISCYEARPER,
         SUM( HSL ) AS HSL
   ...
   GROUP BY RACCT, OBJNR, BLART, FISCYEARPER.
...
*-- Current 예산
  LOOP AT LT_ZCOT0040 ASSIGNING FIELD-SYMBOL(<LS_ZCOT0040>).
    ...
    PERFORM READ_TEXT TABLES   LT_ZCOT0030
                      USING    <LS_ZCOT0040>-ROBJNR
                      CHANGING GS_OUTTAB
                               LV_DELETE.
    ...
    LOOP AT GT_MONTH.
      ...
      LV_FIELDNAME = 'B' && GT_MONTH-V1.

      ASSIGN COMPONENT LV_FIELDNAME OF STRUCTURE GS_OUTTAB
           TO FIELD-SYMBOL(<FS_BUDGET>).
      <FS_BUDGET> = <FS_HSL>.
  ...
  LOOP AT GT_OUTTAB INTO GS_OUTTAB.
    ...
    LOOP AT GT_MONTH.
      ...
      ASSIGN COMPONENT LV_FIELDNAME OF STRUCTURE GS_OUTTAB
           TO FIELD-SYMBOL(<FS_F1>).

      LV_FYEAR = PA_GJAHR && '0' && GT_MONTH-V1.

      READ TABLE  LT_ACDOCA ASSIGNING  FIELD-SYMBOL(<LS_ACDOCA_DD>)
                                WITH KEY RACCT  = GS_OUTTAB-KSTAR
                                         OBJNR  = GS_OUTTAB-OBJNR
                                         BLART  = 'DD'
                                         FISCYEARPER = LV_FYEAR.

      IF SY-SUBRC = 0.
        <FS_F1> = <FS_F1> - <LS_ACDOCA_DD>-HSL.
      ENDIF.

      ASSIGN COMPONENT LV_FIELDNAME OF STRUCTURE GS_OUTTAB
           TO FIELD-SYMBOL(<FS_F2>).
      ...
      IF SY-SUBRC = 0.
        <FS_F2> = <FS_F2> - <LS_ACDOCA_SS>-HSL.
      ENDIF.

    ENDLOOP.

    GS_OUTTAB-A01 = GS_OUTTAB-B01 - GS_OUTTAB-F01.
    GS_OUTTAB-A02 = GS_OUTTAB-B02 - GS_OUTTAB-F02.
    GS_OUTTAB-A03 = GS_OUTTAB-B03 - GS_OUTTAB-F03.
    GS_OUTTAB-A04 = GS_OUTTAB-B04 - GS_OUTTAB-F04.
    GS_OUTTAB-A05 = GS_OUTTAB-B05 - GS_OUTTAB-F05.
    GS_OUTTAB-A06 = GS_OUTTAB-B06 - GS_OUTTAB-F06.
    GS_OUTTAB-A07 = GS_OUTTAB-B07 - GS_OUTTAB-F07.
    GS_OUTTAB-A08 = GS_OUTTAB-B08 - GS_OUTTAB-F08.
    GS_OUTTAB-A09 = GS_OUTTAB-B09 - GS_OUTTAB-F09.
    GS_OUTTAB-A10 = GS_OUTTAB-B10 - GS_OUTTAB-F10.
    GS_OUTTAB-A11 = GS_OUTTAB-B11 - GS_OUTTAB-F11.
    GS_OUTTAB-A12 = GS_OUTTAB-B12 - GS_OUTTAB-F12.

    GS_OUTTAB-ASUM = GS_OUTTAB-A01 +
                     GS_OUTTAB-A02 +
                     GS_OUTTAB-A03 +
                     GS_OUTTAB-A04 +
                     GS_OUTTAB-A05 +
...
FORM SET_COLUMNS_TECHNICAL USING IR_COLUMNS TYPE REF TO
  ...
  TRY.
      LOOP AT GT_COLUMN_REF
        ...
        IF ( <COLUMN_REF>-COLUMNNAME CP 'B*' OR
             <COLUMN_REF>-COLUMNNAME CP 'A*' OR
             <COLUMN_REF>-COLUMNNAME CP 'F*' ).
...
FORM SET_COLUMN_TEXT USING P_COLUMNNAME
  ...
  TRY.
      ...
      IF P_COLUMNNAME CP 'B*' AND
         P_COLUMNNAME  <> 'BSUM' .

        CONCATENATE P_COLUMNNAME+1(2) TEXT-T05 TEXT-C20
              INTO P_COLUMN_TEXT SEPARATED BY SPACE.
      ...
      IF P_COLUMNNAME CP 'A*' AND
         P_COLUMNNAME  <> 'ASUM' .

        CONCATENATE P_COLUMNNAME+1(2) TEXT-T05 TEXT-C21
              INTO P_COLUMN_TEXT SEPARATED BY SPACE.
      ...
      IF P_COLUMNNAME CP 'F*' AND
         P_COLUMNNAME  <> 'FSUM' .

        CONCATENATE P_COLUMNNAME+1(2) TEXT-T05 TEXT-C23
              INTO P_COLUMN_TEXT SEPARATED BY SPACE.
...
*& Form READ_TEXT
*&---------------------------------------------------------------------*
FORM READ_TEXT  TABLES   PT_ZCOT0030 STRUCTURE ZCOT0030
                USING    P_OBJNR TYPE J_OBJNR
                CHANGING PS_OUTTAB STRUCTURE ZCOS0280
                         P_DELETE.
  ...
  CASE P_OBJNR(2).
    ...
    WHEN 'PR'.
      ...
      IF PA_CHECK = ABAP_TRUE.
        ...
        READ TABLE PT_ZCOT0030
                WITH KEY POSID = PS_OUTTAB-POSID
                TRANSPORTING NO FIELDS.
