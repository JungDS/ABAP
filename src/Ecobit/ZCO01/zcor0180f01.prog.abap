...
FORM SELECTED_DATA_RTN .
  ...
  SELECT A~VERSN, A~WRTTP, A~GJAHR, P~PBUKR AS BUKRS,
   ...
   WHERE A~LEDNR = '00'
     ...
     AND P~PBUKR IN @SO_BUKRS
   GROUP BY A~VERSN, A~WRTTP, A~GJAHR, P~PBUKR,
            A~OBJNR, A~KSTAR, B~KTEXT
   UNION ALL
  SELECT A~VERSN, A~WRTTP, A~GJAHR,
  ...
  SELECT A~RVERS  AS VERSN, A~WRTTP, A~RYEAR AS GJAHR,
   ...
   WHERE A~RVERS  = '000'
     ...
     AND P~PBUKR  IN @SO_BUKRS
   GROUP BY A~RVERS, A~WRTTP, A~RYEAR, P~PBUKR,
            A~ROBJNR, A~RKSTAR, B~KTEXT

   UNION ALL
  SELECT A~RVERS , A~WRTTP, A~RYEAR AS GJAHR, A~BUKRS,
   ...
   GROUP BY A~RVERS, A~WRTTP, A~RYEAR, A~BUKRS,
            A~OBJNR, A~SAKNR, B~TXT20

    INTO TABLE @GT_DATA.

*-- 관계사간 거래 ADD
  SELECT A~SPMON, A~SAKNR, A~TXT20, B~PBUKR AS BUKRS, B~OBJNR,
   ...
   GROUP BY A~SPMON, A~SAKNR, A~TXT20, B~PBUKR, B~OBJNR.
...
*-- 금액이 0인 WBS 삭제 처리
  LOOP AT GT_PRPS.
    ...
    ELSE.
      ...
      MOVE GT_PRPS-PBUKR TO GT_BASE-BUKRS.
  ...
  CASE SENDER.
    ...
    WHEN GR_TREE_DATA.
      ...
      IF P_FIELDNAME CP 'S_*'.
        ...
        READ TABLE GT_BASE WITH KEY BUKRS = P_FIELDNAME+2(4).
                ...
                WHEN OTHERS.
                  ...
                  IF SY-SUBRC <> 0.
                    ...
                    LOOP AT GT_BASE.
                      ...
                      LV_FIELDNAME = 'P_' && GT_BASE-BUKRS.
                      PERFORM CLEAR_FIELD USING LV_FIELDNAME
                                                <GS_LINE>.

                      LV_FIELDNAME = 'S_' && GT_BASE-BUKRS.
                      PERFORM CLEAR_FIELD USING LV_FIELDNAME
                                                <GS_LINE>.
                    ...
                    IF SO_BUKRS[] IS INITIAL.
                      ...
                      PERFORM CLEAR_FIELD USING 'P_ZZZZ'
                                                <GS_LINE>.

                      PERFORM CLEAR_FIELD USING 'S_ZZZZ'
                                                <GS_LINE>.

                    ENDIF.

                    PERFORM CLEAR_FIELD USING 'P_SUM'
                                              <GS_LINE>.

                    PERFORM CLEAR_FIELD USING 'S_SUM'
                                              <GS_LINE>.
...
*-- 그룹에 등록되어 있는 계정만큼
              ...
              LOOP AT GT_VALUES WHERE SETID = GT_NODES-SETID.
                ...
                LOOP AT GT_CSKA WHERE KSTAR >= GT_VALUES-VFROM
                  ...
                  LOOP AT GT_DATA ASSIGNING FIELD-SYMBOL(<LS_DATA>)
                    ...
                    IF GT_CSKA-KSTAR CP '04*' OR   "매출 계정은 음수로 표시
                         GT_CSKA-KSTAR CP '0701*' OR
                         GT_CSKA-KSTAR CP '0703*' OR
                         GT_CSKA-KSTAR CP '0705*'.
                    ...
                    IF <FS_PLAN> IS  ASSIGNED.
                      ASSIGN COMPONENT 'P_SUM'
                       OF STRUCTURE <GS_LINE> TO FIELD-SYMBOL(<FS_PSUM>).

                      <FS_PSUM> = <FS_PSUM> + <FS_PLAN>.
                    ENDIF.

                    IF <FS_HSL> IS ASSIGNED.
                      ASSIGN COMPONENT 'S_SUM'
                       OF STRUCTURE <GS_LINE> TO FIELD-SYMBOL(<FS_SSUM>).

                      <FS_SSUM> = <FS_SSUM> + <FS_HSL>.
                    ENDIF.
  ...
  LOOP AT <GT_TABLE> ASSIGNING <GS_LINE>.
    ...
    ASSIGN COMPONENT 'GUBUN' OF STRUCTURE <GS_LINE> TO <FS_VALUE>.
    CHECK <FS_VALUE> = '1'.

    ASSIGN COMPONENT 'ZCODE' OF STRUCTURE <GS_LINE> TO <FS_VALUE>.
      ...
      IF SY-SUBRC = 0.
        ...
        DO LV_LINES TIMES.
          ...
          IF LV_INDEX = 1.
            ...
            READ TABLE <GT_TABLE> ASSIGNING FIELD-SYMBOL(<LS_LINE_TEMP>)
            ...
            IF SY-SUBRC = 0.
              ...
              LOOP AT GT_BASE.
                ...
                PERFORM SET_COPY USING     LV_FIELDNAME
                                 CHANGING  <GS_LINE>
                                           <LS_LINE_TEMP>.
              ...
              IF SO_BUKRS[] IS INITIAL.
                ...
                PERFORM SET_COPY USING    'P_ZZZZ'
                                 CHANGING  <GS_LINE>
                                           <LS_LINE_TEMP>.

                PERFORM SET_COPY USING    'S_ZZZZ'
                                 CHANGING  <GS_LINE>
                                           <LS_LINE_TEMP>.

              ENDIF.

              PERFORM SET_COPY USING    'P_SUM'
                               CHANGING  <GS_LINE>
                                         <LS_LINE_TEMP>.

              PERFORM SET_COPY USING    'S_SUM'
                               CHANGING  <GS_LINE>
                                         <LS_LINE_TEMP>.
          ...
          ELSE.
            ...
            IF SY-SUBRC = 0.
              ...
              LOOP AT GT_BASE.
                ...
                PERFORM CALC_SET USING   LV_FIELDNAME
                                          LV_OPE
                                 CHANGING <GS_LINE>
                                          <LS_LINE_DATA>.
              ...
              IF SO_BUKRS[] IS INITIAL.
                ...
                PERFORM CALC_SET USING   'P_ZZZZ'
                                          LV_OPE
                                 CHANGING <GS_LINE>
                                          <LS_LINE_DATA>.


                PERFORM CALC_SET USING   'S_ZZZZ'
                                          LV_OPE
                                 CHANGING <GS_LINE>
                                          <LS_LINE_DATA>.
            ...
            PERFORM CALC_SET USING   'P_SUM'
                                      LV_OPE
                             CHANGING <GS_LINE>
                                      <LS_LINE_DATA>.


            PERFORM CALC_SET USING   'S_SUM'
                                      LV_OPE
                             CHANGING <GS_LINE>
                                      <LS_LINE_DATA>.

            MOVE GT_ZCOT0150-CALCU+LS_RESULT-OFFSET(1)
              TO LV_OPE.
...
FORM MAKE_TABLE .
  ...
  IF SO_BUKRS[] IS INITIAL.       "기타

    LV_FIELDNAME = 'P_' && 'ZZZZ'.
    LV_TITLE     = TEXT-T09 && '(' && TEXT-T07 && ')'.
    ...
    LV_FIELDNAME = 'S_' && 'ZZZZ'.
    LV_TITLE     = TEXT-T09 && '(' && TEXT-T08 && ')'.
...
*& Form CALC_SET
*&---------------------------------------------------------------------*
FORM CALC_SET  USING    VALUE(P_FIELD)
                         VALUE(P_OPE)
               CHANGING PS_LINE
                        PS_LINES_DATA.

  ASSIGN COMPONENT P_FIELD
      OF STRUCTURE PS_LINE TO FIELD-SYMBOL(<FS_VALUE>).

  ASSIGN COMPONENT P_FIELD
     OF STRUCTURE PS_LINES_DATA TO FIELD-SYMBOL(<FS_VALUE2>).
...
*& Form SET_COPY
*&---------------------------------------------------------------------*
FORM SET_COPY  USING    VALUE(P_FIELD)
               CHANGING PS_LINE
                        PS_LINE_TEMP.

  ASSIGN COMPONENT P_FIELD OF STRUCTURE PS_LINE
      TO FIELD-SYMBOL(<FS_VALUE>).

  ASSIGN COMPONENT P_FIELD OF STRUCTURE PS_LINE_TEMP
