...
FORM SCRFIELDS_FUNCTXT .
  ...
  SSCRFIELDS = VALUE #(
    ...
    FUNCTXT_02 = VALUE SMP_DYNTXT( TEXT      = '사업계획 수행절차 등록'(S03) )
  ).
...
*.. PV_TCODE 점검
  ...
  SET PARAMETER ID 'BDTJ'  FIELD P_GJAHR.
  SET PARAMETER ID 'MLB'   FIELD P_GJAHR.
  SET PARAMETER ID 'ACJ'   FIELD P_GJAHR.
*
  SET PARAMETER ID 'ACM'   FIELD '001'.
  SET PARAMETER ID 'VPE'   FIELD '001'.
  SET PARAMETER ID 'MLP'   FIELD '001'.
  SET PARAMETER ID 'BPE'   FIELD '012'.
  SET PARAMETER ID 'POPR'  FIELD '001'.
  SET PARAMETER ID 'ACPR'  FIELD '01'.
  SET PARAMETER ID 'ACM'   FIELD '001'.
  SET PARAMETER ID '74P'   FIELD '001'.

  SET PARAMETER ID 'KVS'   FIELD P_VERSI.
  SET PARAMETER ID 'PVS'   FIELD P_VERSI.
...
*
*    ASSIGN COMPONENT LV_FIELD OF STRUCTURE LS_0190
*                              TO FIELD-SYMBOL(<FS_CLOSE>).
*
*    CHECK SY-SUBRC EQ 0.
*
*    IF <FS_CLOSE> EQ ABAP_ON.
*      GV_CLOSE = '사업계획 확정'(M03).
*      EXIT.
*    ENDIF.
*  ENDLOOP.
