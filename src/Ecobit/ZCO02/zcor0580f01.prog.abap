...
FORM CHECK_VERSN  USING PV_VERSN.
  ...
  SELECT SINGLE TXT
   ...
   WHERE A~KOKRS    EQ @P_KOKRS
     AND A~VERSN    EQ @P_VERSN
     AND A~ACTUAL   EQ @SPACE
     AND A~PLANNING EQ @GC_X
    INTO @GV_VERSN_TXT.

  IF SY-SUBRC NE 0.
    " & 필드값이 유효하지 않습니다.
    MESSAGE E027 WITH '계획버젼'(S03).
  ENDIF.
...
FORM WRITE_DOCUMENT .
  ...
  DATA LR_COLUMN3 TYPE REF TO CL_DD_AREA.
  DATA LR_COLUMN4 TYPE REF TO CL_DD_AREA.

  PERFORM ADD_TABLE  USING LR_TABLE 4 '100%'.
  PERFORM ADD_COLUMN USING LR_TABLE LR_COLUMN1 '150px'.
  PERFORM ADD_COLUMN USING LR_TABLE LR_COLUMN2 '300px'.
  PERFORM ADD_COLUMN USING LR_TABLE LR_COLUMN3 '150px'.
  PERFORM ADD_COLUMN USING LR_TABLE LR_COLUMN4 '*'.
...
*-- 변경대상 수 : N
  ...
  LR_COLUMN3->ADD_ICON( 'ICON_YELLOW_LIGHT' ).
  LR_COLUMN3->ADD_TEXT( TEXT = LV_TEXT1 SAP_EMPHASIS = CL_DD_AREA=>STRONG ).
  LR_COLUMN4->ADD_TEXT( TEXT = LV_TEXT2 ).
...
*-- 변경완료 수 : N
  ...
  LR_TABLE->NEW_ROW( ).
  LR_COLUMN3->ADD_ICON( 'ICON_GREEN_LIGHT' ).
  LR_COLUMN3->ADD_TEXT( TEXT = LV_TEXT1 SAP_EMPHASIS = CL_DD_AREA=>STRONG ).
  LR_COLUMN4->ADD_TEXT( TEXT = LV_TEXT2 ).
...
*-- 매핑오류 수 : N
  ...
  LR_TABLE->NEW_ROW( ).
  LR_COLUMN3->ADD_ICON( 'ICON_RED_LIGHT' ).
  LR_COLUMN3->ADD_TEXT( TEXT = LV_TEXT1 SAP_EMPHASIS = CL_DD_AREA=>STRONG ).
  LR_COLUMN4->ADD_TEXT( TEXT = LV_TEXT2 ).
...
FORM BDC_FILL .
  ...
  PERFORM BDC_FILL_FLD USING: 'BDC_OKCODE'    '=KOAP'.

  " 계획버전 선택
  PERFORM BDC_FILL_PGM USING  'SAPLSPO4'      '0300'.
  PERFORM BDC_FILL_FLD USING: 'BDC_OKCODE'    '=FURT',
                              'SVALD-VALUE(01)' P_VERSN.
  ...
  IF SY-SUBRC EQ 0.
    ...
    LOOP AT GT_COBRB INTO GS_COBRB FROM SY-TABIX.
      ...
      IF    GS_COBRB-AVORG EQ 'KOAP'  " 정산계획
        AND GS_COBRB-VERSN EQ P_VERSN.
