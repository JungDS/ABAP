...
FORM SET_HIERARCHY_INFO .
  ...
  DATA LS_SETH_NODE     LIKE LINE OF GT_SETH_NODE.
  DATA LS_SETH_NODE_UP  LIKE LINE OF GT_SETH_NODE.
  DATA LS_SETH_VAL      LIKE LINE OF GT_SETH_VAL.
  DATA LV_TABIX         TYPE SY-TABIX.


  CLEAR LS_SETH_VAL.

  READ TABLE GT_SETH_VAL INTO LS_SETH_VAL
                         WITH KEY VFROM = GS_DISPLAY-KOSTL
                                  BINARY SEARCH.

  IF SY-SUBRC NE 0.
    LOOP AT GT_SETH_VAL INTO LS_SETH_VAL.
      CHECK GS_DISPLAY-KOSTL BETWEEN LS_SETH_VAL-VFROM
                                 AND LS_SETH_VAL-VTO.
      EXIT.
    ENDLOOP.
    IF SY-SUBRC NE 0.
  ...
  CHECK LS_SETH_VAL IS NOT INITIAL.

  " 하위그룹
  GS_DISPLAY-SETID2 = LS_SETH_VAL-SETID+8.

  CLEAR LS_SETH_NODE.
  READ TABLE GT_SETH_NODE INTO LS_SETH_NODE
                          WITH KEY SETID = LS_SETH_VAL-SETID.
  CHECK SY-SUBRC EQ 0.

  " 하위그룹내역
  GS_DISPLAY-SETTX2 = LS_SETH_NODE-DESCRIPT.

  LV_TABIX = SY-TABIX.
  WHILE LV_TABIX GT 1.
    LV_TABIX = LV_TABIX - 1.
    READ TABLE GT_SETH_NODE INTO LS_SETH_NODE_UP INDEX LV_TABIX.

    " 동일 레벨의 노드인지, 상위 레벨의 노드인지 점검
    CHECK LS_SETH_NODE_UP-HLEVEL < LS_SETH_NODE-HLEVEL.

    " 상위그룹 & 상위그룹내역
    GS_DISPLAY-SETID1 = LS_SETH_NODE_UP-SETID+8.
    GS_DISPLAY-SETTX1 = LS_SETH_NODE_UP-DESCRIPT.
