        ...
        IF GV_ANSWER = '1'.
          ...
          SELECT @PA_GJAHR  AS GJAHR, A~CTYPE, A~FKSTAR, A~CPERD,
            ...
            LEFT JOIN ZCOT1130T AS C
             ...
             AND C~CTYPE = @PA_CTYPE
           WHERE A~KOKRS = @PA_KOKRS
             AND A~GJAHR = ( SELECT MAX( GJAHR ) FROM ZCOT0010b
...
FORM SAVE_DATA_RTN .
  ...
  DATA: LT_ZCOT0010b TYPE TABLE OF ZCOT0010b WITH HEADER LINE.

  LOOP AT GT_DISPLAY INTO GS_DISPLAY.

    MOVE-CORRESPONDING GS_DISPLAY TO LT_ZCOT0010b.

    LT_ZCOT0010b-ERDAT  = SY-DATUM.
    LT_ZCOT0010b-ERZET  = SY-UZEIT.
    LT_ZCOT0010b-ERNAM  = SY-UNAME.
    LT_ZCOT0010b-AEDAT  = SY-DATUM.
    LT_ZCOT0010b-AEZET  = SY-UZEIT.
    LT_ZCOT0010b-AENAM  = SY-UNAME.
    LT_ZCOT0010b-KOKRS  = PA_KOKRS.
  ...
  TRY .
      ...
      DELETE FROM ZCOT0010b WHERE KOKRS  = @PA_KOKRS
                             AND GJAHR  = @PA_GJAHR
                             AND CTYPE  = @PA_CTYPE.
*                             AND CTYPE  = @GV_CTYPE.

      INSERT ZCOT0010b FROM TABLE LT_ZCOT0010b.
