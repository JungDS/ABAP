*---------------------------------------------------------------------*
*    program for:   VIEWFRAME_ZCOV000
*   generation date: 2021.08.31 at 16:45:40
*   view maintenance generator version: #001407#
*---------------------------------------------------------------------*
FUNCTION VIEWFRAME_ZCOV000             .
...
*----------------------------------------------------------------------*
CALL FUNCTION 'VIEWPROC_ZCOV000'
         EXPORTING
              FCODE          = READ
...
CALL FUNCTION 'VIEWPROC_ZCOV000'
             ...
             IMPORTING
                  UCOMM           = FUNCTION
UPDATE_REQUIRED = STATUS_ZCOV000-UPD_FLAG
             TABLES
                  EXCL_CUA_FUNCT  = EXCL_CUA_FUNCT
EXTRACT = ZCOV000_EXTRACT
TOTAL = ZCOV000_TOTAL
...
*----------------------------------------------------------------------*
      ...
      IF ( FUNCTION EQ SWITCH_TO_SHOW_MODE OR
           ...
           FUNCTION EQ END ) AND
STATUS_ZCOV000-UPD_FLAG NE SPACE.
        PERFORM BEENDEN.
        CASE SY-SUBRC.
          WHEN 0.
CALL FUNCTION 'VIEWPROC_ZCOV000'
...
TOTAL = ZCOV000_TOTAL
            ...
            CASE SY-SUBRC.
              WHEN 0.
IF STATUS_ZCOV000-UPD_FLAG EQ SPACE. EXIT. ENDIF.
...
IF STATUS_ZCOV000-UPD_FLAG NE SPACE.
